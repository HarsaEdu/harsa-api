...
func (courseHandler *CourseHandlerImpl) CreateIntructure(ctx echo.Context) error {
	courseCreateRequest := web.CourseCreateRequestIntructure{}
	err := ctx.Bind(&courseCreateRequest)
	if err != nil {
		return res.StatusBadRequest(ctx, "failed to bind course request", err)
	}
	...
	err = courseHandler.CourseService.CreateIntructure(ctx, &courseCreateRequest, instructorId)
	if err != nil {
		if strings.Contains(err.Error(), "validation") {
			return validation.ValidationError(ctx, err)
		}
