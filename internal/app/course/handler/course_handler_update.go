...
func (courseHandler *CourseHandlerImpl) UpdateIntructure(ctx echo.Context) error {
	idParam := ctx.Param("id")
	...
	request := web.CourseUpdateRequestIntructure{}
	err = ctx.Bind(&request)
	if err != nil {
		return res.StatusBadRequest(ctx, "failed to bind request: ", err)
	}

	err = courseHandler.CourseService.UpdateIntructure(uint(id), uint(user_id), roleString, &request)
	if err != nil {
		if strings.Contains(err.Error(), "validation") {
			return validation.ValidationError(ctx, err)
		}
