...
func (submissionAnswerRepository *SubmissionAnswerRepositoryImpl) Get(offset, limit int, search string, submissionID uint) ([]domain.SubmissionsAnswerDetail, int64, error) {
	...
	query := submissionAnswerRepository.DB.Model(&submissionAnswer).Select(" submission_answers.id as id,user_profiles.first_name as first_name, user_profiles.last_name as last_name  ,submission_answers.status as status").Joins("JOIN user_profiles on user_profiles.user_id=submission_answers.user_id").Where("submission_answers.submisssion_id = ?",  submissionID)
