...
func (moduleRoutes *ModuleRoutesImpl) ModuleWeb(coursesGruop *echo.Group) {
	modulesGroup := coursesGruop.Group("/:courseId/section/modules")
	sectionGroup := coursesGruop.Group("/:courseId/section")

	modulesGroup.POST("", moduleRoutes.ModuleHandler.CreateModule, middleware.InstructorMiddleware)
	...
	modulesGroup.GET("/:id", moduleRoutes.ModuleHandler.GetModuleById, middleware.InstructorMiddleware)
	modulesGroup.PUT("/:id", moduleRoutes.ModuleHandler.UpdateModule, middleware.InstructorMiddleware)
	sectionGroup.PUT("/:id", moduleRoutes.ModuleHandler.UpdateSection, middleware.InstructorMiddleware)
	modulesGroup.DELETE("/:id", moduleRoutes.ModuleHandler.DeleteModule, middleware.InstructorMiddleware)
	sectionGroup.DELETE("/:id", moduleRoutes.ModuleHandler.DeleteSection, middleware.InstructorMiddleware)
	modulesGroup.PUT("/order", moduleRoutes.ModuleHandler.UpdateModuleOrder, middleware.InstructorMiddleware)
